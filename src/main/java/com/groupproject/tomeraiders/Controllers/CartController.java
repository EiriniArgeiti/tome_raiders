...
import com.groupproject.tomeraiders.models.Cart;
...
public class CartController {
    ...
    public String add(@PathVariable int id, HttpSession session, Model model,
        ...
        if (session.getAttribute("cart") == null) {
            ...
            cart.put(id, new Cart(id, book.getTitle(), book.getPrice(), 1, book.getImage()));
            session.setAttribute("cart", cart);
        } else {
            HashMap<Integer, Cart> cart = (HashMap<Integer, Cart>) session.getAttribute("cart");
            if (cart.containsKey(id)) {
                int qty = cart.get(id).getQuantity();
                cart.put(id, new Cart(id, book.getTitle(), book.getPrice(), ++qty, book.getImage()));
            } else {
                cart.put(id, new Cart(id, book.getTitle(), book.getPrice(), 1, book.getImage()));
                session.setAttribute("cart", cart);
            }
